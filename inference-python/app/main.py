...
async_redis_pool = redis_async.ConnectionPool(
    host=REDIS_HOST, port=REDIS_PORT, db=0, decode_responses=True, max_connections=100
)
    ...
    try:
        while True:
            ...
            with tracer.start_as_current_span(
                "process_loan_event", context=ctx
            ) as span:
                span.set_attribute("event_id", event_id)

                if not await async_redis_client.setnx(
                    f"processed_event:{event_id}", "1"
                ):
                ...
                try:
                    ...
                    prob, shap_vals = model_instance.predict(
                        loan_app.amount, loan_app.termMonths
                    )

                    logger.info(
                        f"Processed loan {loan_app.id}: Default Prob={prob:.4f}"
                    )
